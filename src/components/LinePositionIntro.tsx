...
export const LinePositionIntro: React.FC<{ findReal?: boolean }> = ({ findReal = false }) => {
  ...
  return <div className="reflection-intro">
    ...
    <div className="lp-line" aria-hidden="true"><span>{t("ด้านเครื่องส่ง")}</span><span>{t("โหลดเดิม")}<br /><b>100 + j50 Ω</b></span><div className="lp-wire"><i style={{ left: `${100 - distance * 200}%` }}>▼</i></div><small>{t("0.5 λ ← ระยะจากโหลด → 0 λ")}</small></div>
    <label className="lp-slider">{t("จุดวัดอยู่ห่างจากโหลด")} {fmtNum(distance, 4)} λ {t("ไปทางเครื่องส่ง")}
      <input type="range" min="0" max="0.5" step="0.001" value={distance} onChange={(event) => setDistance(Number(event.target.value))} />
    </label>
    <p className="ri-small">{t("ลากตัวเลื่อนไปทางขวา = เพิ่มระยะจากโหลด จุดวัดบนรูปจึงขยับไปทางเครื่องส่งด้านซ้าย · λ อ่านว่า “แลมบ์ดา” คือความยาวคลื่นในสาย")}</p>
    <div className="ri-choices" role="group" aria-label={t("ตำแหน่งวัดตัวอย่าง")}>
    ...
    <div className="ri-reading"><div aria-live="polite"><h4>{t("ค่าที่อ่านได้:")} {textZ(distance)}</h4><p><b>R = {fmtNum(value.re, 2)} Ω</b> {t("คือส่วนต้านทาน")}<br /><b>X = {fmtNum(Math.abs(value.im) < 1e-8 ? 0 : value.im, 2)} Ω</b> {t("คือส่วนรีแอกแตนซ์")}</p><p>{Math.abs(value.im) < 1e-8 ? 'ตอนนี้ X = 0: ค่าที่มองเห็นเป็นความต้านทานล้วน' : value.im > 0 ? 'X เป็นบวก: ค่าที่มองเห็นมีลักษณะเหนี่ยวนำ อยู่ครึ่งบนของกราฟ' : 'X เป็นลบ: ค่าที่มองเห็นมีลักษณะเก็บประจุ อยู่ครึ่งล่างของกราฟ'}</p><p><b>SWR = {fmtNum(swr, 2)} {t("เท่าเดิม")}</b> {t("ตลอดทาง เพราะตัวอย่างนี้ใช้สายไร้การสูญเสียและอ้างอิง 50 Ω เหมือนกัน")}</p></div><div><SmithFigure points={[{ z: normalize(load, 50), label: 'โหลด', cls: 'load' }, { z: normalize(value, 50), label: 'จุดวัด', cls: 'in' }]} curves={[{ zs: path, cls: 'net', arrow: distance > 0 }]} swr={[swr]} xCircles={findReal ? [0] : undefined} /><p className="ri-small">{t("จุดแดง = โหลด • จุดเขียว = ค่าที่จุดวัด • เส้นม่วง = ทางที่จุดเดิน")}<br />{t("เมื่อเริ่มที่ 0 λ หรือครบ 0.5 λ จุดทั้งสองซ้อนกัน")}</p></div></div>
    ...
    {!findReal && <details className="ri-formulas"><summary>{t("ดูสูตรที่อยู่เบื้องหลังภาพ (อ่านภายหลังได้)")}</summary><p>{t("l คือระยะจากโหลดไปทางเครื่องส่ง ส่วน β = 2π/λ ใช้แปลงระยะเป็นมุมทางไฟฟ้า สูตรนี้ใช้กับสายสม่ำเสมอไร้การสูญเสีย")}</p><Tex block tex={'Z_{in}(l)=Z_0\\frac{Z_L+jZ_0\\tan(\\beta l)}{Z_0+jZ_L\\tan(\\beta l)}'} /><Tex block tex={'Z_{in}(\\lambda/4)=\\frac{Z_0^2}{Z_L}, \\qquad Z_{in}(\\lambda/2)=Z_L'} /><p>{t("ที่หนึ่งในสี่คลื่นต้องใช้ Z₀²/ZL ไม่ใช่กลับเศษส่วนของค่าหน่วยโอห์มอย่างเดียว เรื่องค่าปกติ z และแอดมิตแตนซ์จะอธิบายในบทต่อไป")}</p></details>}
  </div>;
};
