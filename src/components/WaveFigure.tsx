...
export const WaveFigure: React.FC<{ gammaMag: number; gammaDeg?: number; len?: number; title?: string }> = ({ gammaMag, gammaDeg = 180, len = 1, title }) => {
  ...
  const yOf = (v: number) => H - pad - (v / (vmax * 1.05)) * (H - 2 * pad);
  ...
  return (
    <div className="miniplot">
      <svg viewBox={`0 0 ${W} ${H}`} width="100%">
        ...
        <rect x={pad} y={pad - 8} width={W - 2 * pad} height={H - 2 * pad + 8} className="mp-bg" />
        {[0, 0.25, 0.5, 0.75, 1].filter((d) => d <= len).map((d) => (
          <g key={d}>
            <line x1={xOf(d)} y1={pad - 8} x2={xOf(d)} y2={H - pad} className="mp-grid" />
        ...
        <text x={pad + 4} y={pad + 4} className="mp-tick">|V| (น้ำเงิน) · |I| (ส้ม) · |Γ| = {fmtNum(gammaMag, 2)} · VSWR = {Number.isFinite(swr) ? fmtNum(swr, 2) : '∞'}</text>
