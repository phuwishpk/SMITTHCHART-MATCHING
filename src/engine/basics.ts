...
/** ตัวเศษและตัวส่วนของ Γ = (z−1)/(z+1) เก็บไว้โชว์ที่มาของมุมในหัวข้อ 5.3 */
const b5num = C(B5_Z.re - 1, B5_Z.im);
const b5den = C(B5_Z.re + 1, B5_Z.im);
...
export const BASICS: Chapter[] = [
  {
    ...
    sections: [
      ...
      {
        id: 'origin', title: 'ใครคิดกราฟนี้ และคิดขึ้นมาทำไม',
        lines: [
          T('Smith Chart คิดขึ้นโดยฟิลลิป เฮการ์ สมิธ (Phillip Hagar Smith, ค.ศ. 1905–1987) วิศวกรไฟฟ้าของ Bell Telephone Laboratories ที่ทำงานด้านสายอากาศและสายส่งสำหรับวิทยุ เขาร่างกราฟรุ่นแรกราวปี 1936–37 ตีพิมพ์ในนิตยสาร Electronics เดือนมกราคม 1939 และปรับปรุงเป็นรูปแบบมาตรฐานที่ใช้ทุกวันนี้ในปี 1944'),
          T('เหตุผลที่คิดขึ้นคือความเบื่อ: การหาอิมพีแดนซ์ที่ต้นสายจากสูตร Z_in = Z₀ (Z_L + jZ₀ tan βl)/(Z₀ + jZ_L tan βl) ต้องคิดจำนวนเชิงซ้อนซ้อนกันด้วยสไลด์รูลทุกครั้งที่ความยาวหรือความถี่เปลี่ยน สมิธสังเกตว่าถ้ามองที่ Γ แทน Z การต่อสายเพิ่มจะเป็นแค่การหมุนรอบจุดกลาง และเส้น r, x คงที่ทั้งหมดกลายเป็นวงกลมที่พิมพ์ลงกระดาษไว้ล่วงหน้าได้ การคำนวณจึงเหลือแค่หาจุดแล้วหมุน'),
          N('กราฟแบบเดียวกันถูกคิดขึ้นโดยอิสระโดย Tosaku Mizuhashi (ญี่ปุ่น, 1937) และ Amiel Volpert (สหภาพโซเวียต, 1939) · ที่ยังใช้ในยุคคอมพิวเตอร์เพราะมองแล้วเห็นทันทีว่าสะท้อนมากไหม ต้องใส่ L หรือ C และต้องเดินไปทางไหน — เครื่องวัด VNA ทุกเครื่องจึงยังแสดง S₁₁ บนกราฟใบนี้'),
        ],
      },
      {
  ...
  chart: { swr: [swrFromGamma(gammaFromz(C(1.2, 0.4))), swrApp], scale: false, fine: false,
    points: [{ z: C(1.2, 0.4), label: 'จุด ก', cls: 'in' }, { z: zApp, label: 'จุด ข', cls: 'load' }] },
          ...
          { kind: 'chart', title: 'อ่าน SWR บนกราฟตัวเต็มด้วยการกวาดรัศมี (แบบเดียวกับที่หน้า Lab วาดให้)',
            ...
            // วาดวงกลม SWR เต็มวงด้วย จะได้เห็นว่าการกวาดรัศมีคือการเดินไปตามวงกลมวงนี้ ไม่ใช่ลากเส้นตรงลงแกน
            swr: [swrFromGamma(gammaFromz(C(0.5, 0.5)))],
            caption: `วงกลมเต็มวงคือวงกลม SWR คงที่ที่ผ่านจุดโหลด ทุกจุดบนวงนี้สะท้อนเท่ากันหมด · เส้นทึบคือรัศมีวงเวียน เส้นโค้งประคือการกวาดไปตามวงนั้นจนแตะแกนนอนครึ่งขวา จุดที่แตะอ่าน r ได้เท่ากับ SWR พอดี · เส้นประที่ลากลงข้างล่างพาไปที่แถบ SWR/RL ใต้กราฟ ซึ่งอ่านค่าที่เหลือได้ในคราวเดียว: |Γ| = ${n(abs(gammaFromz(C(0.5, 0.5))), 3)} · SWR = ${n(swrFromGamma(gammaFromz(C(0.5, 0.5))), 2)} · RL = ${n(returnLossDb(gammaFromz(C(0.5, 0.5))), 1)} dB (คำนวณโดยแอป) · แถบด้านล่างวางตามผังมาตรฐาน: กว้างเท่าเส้นผ่านศูนย์กลาง จุด CENTER อยู่ใต้จุดกลางกราฟพอดี ระยะเท่ารัศมีวัดจาก CENTER ไปทางซ้ายอ่าน SWR / return loss / |Γ| วัดไปทางขวาอ่าน loss / transmission · เส้นประจึงลากดิ่งลงมาชนจุดอ่านค่าได้ตรง ๆ ทั้งสองข้าง` },
        ],
      },
      {
  ...
  hint: 'แทนลงสูตร ∠Γ = 180° − 720° × (ค่าที่อ่านได้)' },
      ...
      },
      {
        ...
        lines: [
          ...
          T(`ก่อนอ่านบรรทัดล่าง ขอเคลียร์เลข ${n(b5ang, 1)}° ก่อน เพราะไม่ได้ตั้งขึ้นเอง แต่คือมุมของ Γ ที่โหลด หาจากสูตรเดิมของบทที่ 2 คือ Γ = (z − 1)/(z + 1) แล้วอ่านมุมของผลลัพธ์`),
          M(`\\Gamma_L = \\frac{z-1}{z+1} = \\frac{${n(b5num.re, 1)} + j${n(b5num.im, 1)}}{${n(b5den.re, 1)} + j${n(b5den.im, 1)}} = \\frac{${n(abs(b5num), 4)}\\,\\angle\\,${n(deg(arg(b5num)), 2)}^\\circ}{${n(abs(b5den), 4)}\\,\\angle\\,${n(deg(arg(b5den)), 2)}^\\circ} = ${n(abs(b5g), 4)}\\,\\angle\\,${n(b5ang, 2)}^\\circ`),
          T(`หารจำนวนเชิงซ้อนคือ "ขนาดหารกัน มุมลบกัน" · มุมจึงเป็น ${n(deg(arg(b5num)), 2)}° − ${n(deg(arg(b5den)), 2)}° = ${n(b5ang, 2)}° ส่วนขนาดเป็น ${n(abs(b5num), 4)} ÷ ${n(abs(b5den), 4)} = ${n(abs(b5g), 4)} ซึ่งก็คือ |Γ| ตัวที่ใช้วาดวง SWR ในบทที่ 4 นั่นเอง (คำนวณโดยแอป)`),
          N(`มุมกับสเกลเป็นค่าเดียวกันแต่คนละหน่วย จึงแปลงกลับไปมาได้ · สเกล toward generator เริ่มนับ 0 ที่ SHORT ซึ่งเป็นมุม 180° แล้วไล่ตามเข็ม ดังนั้น สเกล = (180° − มุม) ÷ 720° · แทน ${n(b5ang, 1)}° ได้ (180 − ${n(b5ang, 1)}) ÷ 720 = ${n(b5wtg, 4)} λ ตรงกับเลขสเกลในบรรทัดบนพอดี`),
