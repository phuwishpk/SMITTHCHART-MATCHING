...
// ---------------------------------------------------------------
import { Circuit, AntennaPoint, buildCircuit } from './circuit';
...
import { gammaFromZ, zFromGamma, normalize, admittance, rotateTowardGenerator, lineInput, swrFromGamma } from './rf';
...
export const ex2Book = () => buildCircuit(52e6, 50, [['tline', 'series', { Z0: 83, len: 0.14, vf: 0.66, lossDb: 0 }], ['antenna', 'series', {}, EX2_TABLE]]);
...
export const COURSE: Chapter[] = [
    ...
    sections: [
      {
        ...
        lines: [
          T('ไฟล์ที่ใช้อ้างอิงมี 157 หน้า: ส่วนต้น (Foreword, Preface, About the Author, Contents, Errata), Introduction, Chapter I–V ครบ และ Chapter VI ถึงช่วงต้น Example 6 (หน้าสุดท้ายคือ 6-39 ซึ่งเป็น schematic ของ matching solution) ส่วน Example 7–11 และ Chapter VII (Construction of Overlay Tracing Box) ไม่มีในไฟล์ แม้สารบัญจะระบุไว้'),
          ...
          N('ภาพในหัวข้อนี้เป็น "ภาพจำลองตามแนวคิดของ Figure ในหนังสือ" คำนวณสดด้วย engine ไม่ใช่ภาพจากหนังสือ ตัวเลขตัวอย่างที่ระบุว่า "จากหนังสือ" คือค่าที่ปรากฏในสรุปเนื้อหา ส่วนค่าที่ระบุว่า "คำนวณโดยแอป" คือค่าที่ engine คำนวณเพิ่มเติม'),
        ],
      },
      {
        ...
        lines: [
          ...
          T('Figure A — Reciprocity theorem: เสาอากาศ A และ B สองต้น กรณี A ส่ง/B รับ กับ B ส่ง/A รับ ให้ผลเหมือนกัน คุณสมบัติ (อิมพีแดนซ์, pattern) ของเสาอากาศจึงเหมือนกันทั้งตอนส่งและรับ'),
          K('A (ส่ง) ))) ~~~~~ ((( B (รับ)\n\nB (ส่ง) ))) ~~~~~ ((( A (รับ)      →  ผลเท่ากัน (reciprocity)'),
    ...
    sections: [
      {
        ...
        lines: [
          ...
          K('Fig. 1-1 (lumped):      Source ──[L]──┬──\n                                     [C]\n                                      ⏚\n\nFig. 1-2 (distributed):  ──R──L──┬──R──L──┬──R──L──┬──\n                                 C  G      C  G      C  G\n                                 ⏚         ⏚         ⏚'),
          ...
          T('ตัวอย่างคำนวณ: สายโคแอกเชียลที่มี L = 250 nH/m และ C = 100 pF/m'),
      ...
      },
      {
        ...
        lines: [
          ...
          T('ตัวอย่าง: โหลด 100 Ω บนสาย 50 Ω → Γ = (100 − 50)/(100 + 50) = 1/3 → VSWR = 2'),
      ...
      },
      {
        ...
        lines: [
          ...
          T('ตัวอย่าง VSWR = 2: |Γ| = 1/3 → กำลังสะท้อน 11.1 % → mismatch loss 0.51 dB · VSWR = 3: |Γ| = 0.5 → สะท้อน 25 % → 1.25 dB'),
          T('เมื่อสายมีการสูญเสีย คลื่นสะท้อนต้องเดินทางไป-กลับ จึงเกิด additional loss ตาม SWR (Fig. 1-10) สูตรที่ใช้คำนวณ (ARRL):'),
    ...
    sections: [
      ...
      },
      {
        ...
        lines: [
          ...
          T('สายสั้นกว่า λ/4 ปลายลัดเป็น inductive, ปลายเปิดเป็น capacitive (Fig. 2-6) การวัดอิมพีแดนซ์ที่ตำแหน่งต่าง ๆ บนสาย (Fig. 2-7) ให้ค่าที่หมุนตามวงกลม SWR คงที่ Fig. 2-8 คือ reactance ของสายปลายลัดเทียบความยาวไฟฟ้า และ Fig. 2-10/2-11 คือ reactance curve ของสตับสำหรับ Z₀ ต่าง ๆ'),
          T('Fig. 2-9: สตับปลายเปิดและปลายลัดยาว 0.15 λ (54°) บน Smith Chart: ปลายเปิด z = −j cot 54° = −j0.727 · ปลายลัด z = +j tan 54° = +j1.376 · Fig. 2-12: Smith Chart ที่ calibrate สเกลรอบนอกเป็นองศาไฟฟ้าของสตับ (1 รอบ = 360° ไฟฟ้า = λ/2)'),
          N('ในแอป: สเกลรอบนอกของ Smith Chart เป็น wavelengths (0–0.5 λ) แปลงเป็นองศาไฟฟ้าได้ด้วย ×720 (0.15 λ = 108° บนกราฟ = 54° ของ βl)'),
        ],
        figures: [
      ...
      },
      {
        ...
        figures: [
          ...
          { kind: 'lab', label: 'Lab: λ/4 transformer Z_t = 173.2 Ω กับโหลด 600 Ω', circuit: qwt600, note: 'กด Explain เพื่อดูขั้น normalize ใหม่ด้วย Z_t' },
        ],
      },
      {
    ...
    sections: [
      ...
      },
      {
        ...
        lines: [
          ...
          T('ตัวอย่างจากหนังสือ: Z = 30 + j20 Ω บนสาย 50 Ω → z = 0.6 + j0.4 → y = (0.6 − j0.4)/0.52 = 1.154 − j0.769 บน Smith Chart จุด y อยู่ตรงข้ามจุด z ผ่านศูนย์กลาง (diametrically opposite) หรืออ่านจากกราฟ admittance (Fig. 3-4) ที่ซ้อนทับ (Fig. 3-8)'),
        ],
        figures: [
          ...
          { kind: 'smith', title: 'ตามแนวคิด Fig. 3-8: ซ้อนกราฟ Y (เขียว) บนกราฟ Z: จุดเดิมอ่านได้ทั้ง z และ y', showY: true, points: [{ z: C(0.6, 0.4), label: 'z = 0.6 + j0.4 / y = 1.15 − j0.77', cls: 'load' }], gCircles: [1.154], bCircles: [-0.769] },
      ...
      },
      {
        ...
        lines: [
          T('series L: เดินตามวงกลม r คงที่ขึ้น (x เพิ่ม) · series C: เดินตามวงกลม r คงที่ลง · shunt C: เดินตามวงกลม g คงที่ตามเข็ม (b เพิ่ม) · shunt L: เดินตามวงกลม g คงที่ทวนเข็ม (b ลด)'),
          N('กฎนี้คือหัวใจของ Chapter IV และ V ทดลองได้ทันทีใน Lab โดยลาก L/C วางอนุกรม/ขนานแล้วเลื่อนสไลเดอร์'),
        ],
        figures: [
          { kind: 'smith', title: 'จุดเริ่ม z = 0.5 + j0.3: ทิศทางของ series L/C และ shunt L/C', showY: true, points: [{ z: C(0.5, 0.3), label: 'z₀', cls: 'load' }], curves: [
            { zs: moveSeries(C(0.5, 0.3), 0.8), cls: 'in', arrow: true, label: 'series L' },
            { zs: moveSeries(C(0.5, 0.3), -0.8), cls: 'net', arrow: true, label: 'series C' },
            { zs: moveShunt(C(0.5, 0.3), 0.9), cls: 'y', arrow: true, label: 'shunt C' },
            { zs: moveShunt(C(0.5, 0.3), -0.9), cls: 'stub', arrow: true, label: 'shunt L' },
          ], caption: 'เขียว = series L (ขึ้นตาม r), ม่วงเข้ม = series C (ลงตาม r), เขียวอมฟ้า = shunt C (ตาม g ตามเข็ม), ม่วง = shunt L (ตาม g ทวนเข็ม)' },
        ],
      },
      {
        ...
        lines: [
          ...
          M('\\text{ตรวจกลับ: } z_{meas} = \\frac{z_L + j\\tan\\beta l}{1 + j z_L \\tan\\beta l},\\quad \\beta l = 2\\pi(0.120) = 43.2^\\circ'),
        ],
        figures: [
  ...
  },
  {
    ...
    intro: 'การเลือก reactive combination, L-type network ทั้ง 8 รูปแบบ (Fig. 4-1 a–h), การแบ่ง Smith Chart เป็น Region 1–4 เพื่อเลือก configuration (Fig. 4-2), ข้อจำกัดของการ match ด้วย element เดียว (Fig. 4-4) และตัวอย่าง Z_A = 80 − j40 Ω → 50 Ω ด้วยสายส่ง + shorted shunt stub (Fig. 4-5 ถึง 4-8)',
    sections: [
      {
        ...
        figures: [
          ...(['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'] as const).map((id, i) => {
            ...
            return { kind: 'circuit', title: `Fig. 4-1(${id}): ${first[1]} ${first[0] === 'inductor' ? 'L' : 'C'} (ชิดโหลด) → ${second[1]} ${second[0] === 'inductor' ? 'L' : 'C'}`, circuit: buildCircuit(100e6, 50, [[second[0], second[1], second[0] === 'inductor' ? { L: 60 } : { C: 30 }], [first[0], first[1], first[0] === 'inductor' ? { L: 60 } : { C: 30 }], ['load', 'series', { R: 100, X: -50 }]]) } as Figure;
          }),
        ],
      },
      {
        ...
        lines: [
          T('Region 1: ภายในวงกลม r = 1 (R_L > Z₀) → ตัวแรกต้องเป็นตัวขนาน (เดินตาม g ไปตัด r = 1) · Region 2: ภายในวงกลม g = 1 (G_L > Y₀) → ตัวแรกต้องเป็นตัวอนุกรม (เดินตาม r ไปตัด g = 1) · Region 3 (ครึ่งบน นอกทั้งสองวงกลม) และ Region 4 (ครึ่งล่าง) → ใช้ได้ทั้งสองแบบ แต่ชนิด L/C ถูกกำหนดโดยทิศที่ต้องเดิน'),
        ...
        figures: [
          { kind: 'smith', title: 'ตามแนวคิด Fig. 4-2/4-7: Region 1 (ใน r = 1), Region 2 (ใน g = 1), Region 3/4 (นอกทั้งสอง)', regions: true, showY: true, rCircles: [1], gCircles: [1], labels: [{ z: C(3, 0.6), text: 'Region 1' }, { z: C(0.33, -0.2), text: 'Region 2' }, { z: C(0.7, 1.6), text: 'Region 3' }, { z: C(0.7, -1.6), text: 'Region 4' }] },
        ],
      },
      {
        ...
        lines: [
          ...
          N('คำตอบที่สองคือ d = 0.295 λ, l = 0.357 λ (ปุ่มเฉลยในแผง PROPERTIES ของสตับแสดงทั้งสองชุด)'),
        ],
        figures: [
          { kind: 'smith', title: 'ตามแนวคิด Fig. 4-8: z_A → y_A → หมุน d → g = 1 → สตับ → ศูนย์กลาง', showY: true, points: [{ z: C(1.6, -0.8), label: 'z_A', cls: 'load' }, { z: zFromGamma(rotateTowardGenerator(gammaFromZ(C(1.6, -0.8), 1), ch4Stub.dLambda)), label: 'y = 1 + jb', cls: 'mid' }, { z: C(1, 0), label: 'match', cls: 'in' }], curves: [{ zs: linspace(0, ch4Stub.dLambda, 30).map((l) => zFromGamma(rotateTowardGenerator(gammaFromZ(C(1.6, -0.8), 1), l))), cls: 'net', arrow: true }, { zs: linspace(0, 1, 30).map((t) => admittance(C(1, ch4Stub.yAtStub.im * (1 - t)))), cls: 'y', arrow: true }], gCircles: [1] },
          { kind: 'lab', label: 'Lab: 80 − j40 Ω + TL(d) + shorted stub (เปิด "วิธีทำบน Smith Chart" ได้จากบทเรียน Level 10)', circuit: ch4Circuit, showY: true },
        ],
      },
    ],
  },
    ...
    sections: [
      {
        ...
        figures: [
          ...
          { kind: 'plot', title: 'reactance/susceptance ปกติเทียบ f/f₀ (ค่า 1 ที่ f₀)', xLabel: 'f / f₀', yLabel: 'ขนาดเทียบกับค่าที่ f₀', xMin: 0.8, xMax: 1.2, yMin: 0.7, yMax: 1.3, series: [{ name: 'X_L, B_C ∝ f', points: linspace(0.8, 1.2, 41).map((r) => [r, r] as [number, number]) }, { name: '|X_C|, |B_L| ∝ 1/f', color: '#dc2626', points: linspace(0.8, 1.2, 41).map((r) => [r, 1 / r] as [number, number]) }], xTicks: [0.8, 0.9, 1, 1.1, 1.2] },
        ],
      },
      {
        id: 'cases', title: 'ทดลอง L-network ครบ 8 แบบกับโหลด 3 ความถี่ (Tables 5-5 ถึง 5-12, Fig. 5-1 ถึง 5-8)',
        lines: [
          T('หนังสือใช้อิมพีแดนซ์ 3 ความถี่ (เช่น 30, 31, 32 MHz) ทำ matching ที่ความถี่กลางให้เข้า 1 + j0 พอดี แล้วดูว่าความถี่ข้างเคียง "blossom" (กระจายออก) แค่ไหน สำหรับวงจร 4-1(a) ถึง (h) — ค่าอิมพีแดนซ์ที่หนังสือใช้ไม่ได้อยู่ในสรุปที่ได้รับ ตารางด้านล่างจึงใช้โหลดของ Example 1 (12.0–12.4 MHz) เป็นตัวอย่างและคำนวณสดโดยแอป'),
        ...
        figures: [
          { kind: 'lcases', title: 'ตัวแก้ 8 กรณีสำหรับโหลด Example 1 ที่ f₀ = 12.2 MHz (คำนวณโดยแอป)', table: EX1_TABLE, f0: 12.2e6, Z0: 50, caption: 'กรณีที่ "ทำไม่ได้" คือเครื่องหมายของค่าที่ต้องใช้ไม่ตรงกับชนิด L/C ของวงจรนั้นสำหรับโหลดนี้ (ดู Region ใน Chapter IV)' },
        ],
      },
    ],
  },
    ...
    sections: [
      {
        ...
        lines: [
          ...
          N('เทียบกับตัวแก้ 8 กรณีของแอป (Chapter V): กรณี (g) shunt L → series L ที่ 12.2 MHz ให้ L_shunt = 1627 nH (≈ 1.63 μH ตรงหนังสือ) และ L_series = 1129 nH — หนังสือเลือก 1.255 μH เพื่อให้ทั้งสามจุดกระจายอยู่ในวงกลม SWR 2 อย่างสมดุล ไม่ใช่ให้ความถี่กลางเข้าศูนย์กลางพอดี'),
        ],
        figures: [
      ...
      },
      {
        ...
        lines: [
          ...
          T(`ตรวจโดยแอป: ค้นหาความยาวสาย 83 Ω ที่ให้ SWR สูงสุดในแบนด์ต่ำสุด ได้ ${'0.140'} λ (ที่ 52 MHz, ความยาวไฟฟ้าเปลี่ยนตาม f) → SWR = ${solveSweep(ex2Book()).map((p) => fmtNum(p.result.swrIn, 2)).join(' / ')} สูงสุด ${fmtNum(sweepMaxSwr(solveSweep(ex2Book())), 2)} ≈ 1.5 ✓ (Z₀′ = 75–90 Ω ให้ผลใกล้เคียง โดย 83–85 Ω ดีที่สุด)`),
          N('สังเกตว่า transformer เส้นเดียวไม่ใช่ λ/4: ความยาวถูกเลือกให้ "หมุน" curve ทั้งเส้นเข้ามาในวงกลม 1.5 (Fig. 6-2b/c)'),
        ],
        figures: [
          { kind: 'table', title: 'ผลก่อน/หลัง (คำนวณโดยแอป)', head: ['f (MHz)', 'z_L', 'z_in หลัง 83-Ω line', '|Γ|', 'SWR'], rows: bandRows(ex2Book()) },
          { kind: 'smith', title: 'ตามแนวคิด Fig. 6-2: curve เดิม (แดง) และหลังผ่านสาย 83 Ω 0.140 λ (เขียว) กับวงกลม SWR 1.5', points: [...sweepPts(ex2Load(), 'in', 'load'), ...sweepPts(ex2Book(), 'in', 'in')], curves: [{ zs: sweepZ(ex2Load(), 'in'), cls: 'load' }, { zs: sweepZ(ex2Book(), 'in'), cls: 'in' }], swr: [1.5] },
          { kind: 'lab', label: 'Lab: Example 2 (สาย 83 Ω 0.140 λ + เสาอากาศ 50–54 MHz)', circuit: ex2Book, swrTarget: 1.5 },
        ],
      },
      {
        ...
        lines: [
          ...
          W('ค่าอิมพีแดนซ์ของเสาอากาศใน Example 3 ไม่ได้อยู่ในสรุปที่ได้รับ จึงยังตรวจตัวเลขไม่ได้ Lab ด้านล่างเป็น "โครงวงจร" ของวิธีแก้ ใส่ค่าอิมพีแดนซ์จากหนังสือลงในตาราง ANT แล้วปรับความยาวสายทั้งสองส่วนได้'),
        ],
        figures: [
          { kind: 'lab', label: 'Lab (โครงวงจร Ex.3): Source — TL 64 Ω — TL 105 Ω — resonant stub 25 Ω (λ/4 @3.75 MHz) — ANT (ใส่ข้อมูลเอง)', circuit: () => buildCircuit(3.75e6, 50, [['tline', 'series', { Z0: 64, len: 0.1, vf: 0.66, lossDb: 0 }], ['tline', 'series', { Z0: 105, len: 0.1, vf: 0.66, lossDb: 0 }], ['stub_short', 'shunt', { Z0: 25, len: 0.25 }], ['antenna', 'series', {}, [{ f: 3.5e6, R: 50, X: -200 }, { f: 3.75e6, R: 65, X: 0 }, { f: 4.0e6, R: 80, X: 200 }]]]), swrTarget: 2, showY: true, note: 'ตาราง ANT ในโครงนี้เป็นค่าสมมุติเชิงคุณภาพ (capacitive → resonant → inductive) ไม่ใช่ค่าจากหนังสือ' },
        ],
      },
      {
        ...
        lines: [
          ...
          T('วิธีแก้ในหนังสือ: เพิ่ม series capacitive reactance → แปลง Z → Y → เพิ่ม shunt capacitive susceptance → กลับเป็น Z → ยังไม่เข้าวงกลม → เพิ่ม resonant series L-C อีกขั้น แล้วปรับ resonance ไม่ให้จุด 3.7 MHz หลุดวงกลม ผลสุดท้ายอยู่ในวงกลมประมาณ SWR 2.2 (Fig. 6-4c)'),
          T('ค่าอุปกรณ์ที่รูปในหนังสือแสดง: series capacitor 150.5 pF, series inductor 13 μH, shunt capacitor 1274 pF และ capacitor ใกล้เสาอากาศ 588 pF'),
          W('ค่าอิมพีแดนซ์ของเสาอากาศและลำดับต่อที่แน่นอนของอุปกรณ์ไม่ได้อยู่ในสรุปที่ได้รับ Lab ด้านล่างเรียงอุปกรณ์ตามลำดับที่สรุประบุ (ยังไม่ยืนยันกับรูปในหนังสือ) และตาราง ANT เป็นค่าสมมุติ'),
        ],
        figures: [
          { kind: 'lab', label: 'Lab (โครงวงจร Ex.4): Source — C 150.5 pF — L 13 μH — C 1274 pF(⏚) — C 588 pF — ANT (ใส่ข้อมูลเอง)', circuit: () => buildCircuit(3.75e6, 50, [['capacitor', 'series', { C: 150.5 }], ['inductor', 'series', { L: 13000 }], ['capacitor', 'shunt', { C: 1274 }], ['capacitor', 'series', { C: 588 }], ['antenna', 'series', {}, [{ f: 3.5e6, R: 30, X: 250 }, { f: 3.75e6, R: 40, X: 320 }, { f: 4.0e6, R: 55, X: 400 }]]]), swrTarget: 2, showY: true, note: 'ค่าในตาราง ANT เป็นค่าสมมุติ (inductive สูง) ไม่ใช่ค่าจากหนังสือ' },
        ],
      },
      {
        ...
        lines: [
          ...
          T(`แปลงความยาวจริงเป็น λ ที่ 29 MHz (VF 0.66, λ = ${fmtNum(LAM29, 3)} m): สาย 2.8 ft = ${fmtNum(EX5_LINE_LEN, 4)} λ, สตับ 5.56 ft = ${fmtNum(EX5_STUB_LEN, 4)} λ ≈ λ/4 (resonant stub)`),
        ...
        figures: [
          ...
          { kind: 'circuit', title: 'Schematic ของ matching network (ตามหนังสือ)', circuit: ex5Full() },
      ...
      },
      {
        ...
        lines: [
          ...
          K('50 Ω line ──┬──[ C ]──[ L ]── antenna\n            │\n         stub (100 Ω, short)\n            ⏚        "Schematic of matching solution." (หน้า 6-39)'),
        ],
        figures: [
          { kind: 'lab', label: 'Lab (โครงวงจร Ex.6): Source — stub 100 Ω(⏚) — C — L — ANT (2 จุดปลายจากหนังสือ + ค่าสมมุติตรงกลาง)', circuit: () => buildCircuit(4e6, 50, [['stub_short', 'shunt', { Z0: 100, len: 0.2 }], ['capacitor', 'series', { C: 400 }], ['inductor', 'series', { L: 2000 }], ['antenna', 'series', {}, [{ f: 2e6, R: 92, X: -90 }, { f: 3e6, R: 120, X: 60 }, { f: 4e6, R: 160, X: 120 }, { f: 5e6, R: 140, X: 40 }, { f: 6e6, R: 115, X: -25 }]]]), swrTarget: 3, showY: true, note: 'จุด 3–5 MHz ในตาราง ANT เป็นค่าสมมุติ (inductive สูงช่วงกลางตามคำอธิบาย) ค่า C, L, สตับ เป็นค่าตั้งต้นให้ปรับเอง' },
        ],
      },
      {
