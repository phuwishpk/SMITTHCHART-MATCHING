...
- Deep link: `?example=ex7` `?lesson=l10` `&step=5` `&all=1` `&y=1` `&sel=0&probe=0.1` `&max=chart` (palette / inspector / canvas / chart / explain) `&sec=<section>` (เลื่อนไปยังหัวข้อในคอร์ส เช่น `?view=course&ch=ch2&sec=qwt`) `?modal=glossary`
